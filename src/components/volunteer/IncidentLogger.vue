<template>
  <div class="w-full">
    <div class="flex items-center gap-2 mb-6">
      ...
      <h2 class="text-xl font-extrabold tracking-tight text-white">{{ $t('logIncident') }}</h2>
    </div>
    
    <form @submit.prevent="submitIncident" class="flex flex-col gap-6">
      
      <div>
        <label for="location-context" class="block text-sm font-semibold text-white/80 mb-1.5">{{ $t('locationContext') }}</label>
        <input 
          id="location-context" 
          type="text" 
      ...
      <div>
        <label class="block text-sm font-semibold text-white/80 mb-1.5">{{ $t('uploadPhotoEvidence') }}</label>
        ...
        <div class="relative group mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-white/20 border-dashed rounded-xl hover:border-indigo-500/50 hover:bg-indigo-500/10 transition-all duration-300 bg-white/5 overflow-hidden min-h-[180px]">
          <div class="space-y-2 text-center flex flex-col items-center justify-center z-10" :class="{'opacity-0': previewImage}">
            ...
            <div class="flex text-sm text-white/60">
              <label for="incident-image" class="relative cursor-pointer bg-transparent rounded-md font-bold text-indigo-400 hover:text-indigo-300 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-[#0a0a1a] focus-within:ring-indigo-500">
                ...
                <input id="incident-image" type="file" accept="image/*" @change="handleFileUpload" :aria-label="$t('uploadPhotoOfIncident')" class="sr-only" />
              </label>
          ...
          <div v-if="previewImage" class="absolute inset-0 z-0">
            <img :src="previewImage" :alt="$t('incidentPreviewThumbnail')" class="w-full h-full object-cover" />
            <div class="absolute inset-0 bg-gradient-to-t from-zinc-900/80 via-zinc-900/20 to-transparent flex flex-col justify-end p-4">
               <div class="flex items-center gap-2 text-white">
                 <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="text-emerald-400"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                 <span class="font-semibold text-sm drop-shadow-md">{{ $t('imageAttached') }}</span>
               </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="isProcessing" class="p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-xl flex items-center gap-3 motion-safe:animate-pulse">
        <svg aria-hidden="true" class="motion-safe:animate-spin h-5 w-5 text-indigo-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
        <p class="font-semibold text-indigo-300 text-sm m-0">{{ $t('visionProcessing') }}</p>
      </div>
      ...
      <BaseButton type="submit" variant="primary" :disabled="!locationContext || isProcessing" block :aria-label="$t('dispatchTeam')" class="mt-2">
        {{ $t('dispatchTeam') }}
      </BaseButton>
    </form>
  </div>
